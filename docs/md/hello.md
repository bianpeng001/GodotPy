...
1. 血条已经做了视野内实例化, 看不到的就没有对应的实例. 接下来要把这个方案, 用在城池和部队上面. 减少实例化的单位数量.
2. 附属建筑, 就是城市附近的建造玩法. 归属于所在地块的城市, 地块没有城的话, 还可以建设城? 地块不做直接攻击, 但随着地块里面敌军的数量, 以及攻城的情况, 附属建筑会收到破坏. 治安差也会导致附属建筑受破坏.
...
1. 部队消灭敌对势力
2. 部队就近进城
3. 改进npc对话, 用队列来做, 避免对话被冲掉. 也能简化使用. wait时长根据字数自动判断
4. 新增兵种, 阵型的字段
5. 探索之后, 是有收益的, 物品和武将. 目前先用个假的代替, 不实际获取
6. 名将列表, 主要用于手动配置名将的数值, 并且先占了名字, 避免随机出现名将的名字, 但数值有明显不匹配的情况
7. 新增一个村落, 比县低一级

TODO

1. 战斗过程里, 发现一些有潜力的小兵, 用于培养
2. 机器人玩家的内政AI
3. 自动战斗, 攻击和反击
4. 存档读档
5. 要设计一套武将技能
6. 完善物品的附加数值
7. 计谋, 选择队伍里面的武将, 对目标单位, 执行一个计略. 不一定有计略, 有的话. 自动执行, 执行期间, 就是相当于发动了一个技能. 播放技能动作和特效完事了.
...
<div class="date">2023年7月18日</div>

1. 战斗, 士兵的模型. 至少要做步兵, 弓兵, 骑兵.
2. 世界地图, 查看全部的势力.
3. 数值成长曲线, 军队的消耗
4. 军队进城
5. 城对军队的攻击, 军队之间的攻击.
6. 城市最多攻击几个队伍?
7. 武将技能, 这就是现在的成语大师. 得有一些技能和动作, 动作如何表现呢?
8. 武将活动, 比如探索,拜访,外交啥的, 出去了得有一阵才能回来

<div class="date">2023年7月11日</div>

1. 自动增长的资源里, 去掉兵力, 改为内政手动征兵. 农业商业治安, 都是手动增加. 并消耗行动力
2. 存档. sqlite里面数组,居然要用字符串来标识,真实令人意外!

<div class="date">2023年7月10日</div>

## 开发日志

1. 数值模型里面, 军队是要有消耗的. 不然小小一个城, 能无限堆兵, 岂不可笑.
2. 然后一旦兵堆起来, 城与城之间, 就需要派运输队. 手工操作台繁杂了, 自动派遣好一些.
...
1. 默认还是六边形, 因为六边形确实好看一些
2. 等到用真实地图的时候, 再开启那个平滑UV
3. 山地, 城市之间的路的方案待定
...
观看一个讲反围剿的战例, 感觉城于城之间的路, 还是很重要的. 毕竟, 大部队不可能翻山越岭地去. 因为有路, 所以才有战略位置的差别, 不然都随便走随便打好了. 才有机会进行伏击.

<div class="date">2023年6月08日</div>

1. 外交还是开战这个, 需要根据友好度来进行
2. 偷袭, 可以破坏友好度

<div class="date">2023年6月05日</div>

1. 武将头像,现在比较要紧了. 资源上的话. 我还是坚持那个拼图方案. 
   1. 2D方案,要手绘一些组件,眼耳口鼻
   2. 3D方案,建模!!
   3. 之前尝试了3D+2D的方式, 挺丑的, 还是算了. 这回还是简单的3D加骨骼方案吧.
2. robot player, 需要活跃一些, 让玩家看到他们的行动
   1. 外交
   2. 贸易
   3. 策略, 比如防火,偷窃,谍报,反间...
   4. 机器人玩家, 拥有多个城池
...
1. 主线改了一个, 改ai的文案, 文字不必是古文
2. 继续robot玩家的活动, 一定要丰富一些. 外交,拜访, 结交啥的.

<div class="date">2023年5月31日</div>

1. 六边形已经很漂亮了
2. 接下来, 想来想去, 最要紧的是AI, 这样就能有来有回了
3. 主线剧情的推动, 用等级来判断. 也可以没有剧情.

<div class="date">2023年5月25日</div>

1. 重新做一下六边形格子的生成, 重点是要把UV的计算, 映射到平滑的上去, 也支持点模式.

<div class="date">2023年5月22日</div>

1. 一个可能有用的简单策略, 策略为高筑墙,广积粮,远交近攻,柿子挑软的捏,防守反击.
2. 内政, 就是用武将的行动力, 换取其他的资源. 但是转换率有点关系, AI可以无视内政数值.
3. 行动力, 满分10, 执行一个指令就-1. 一定事件后恢复
4. 特殊事件, 是一个很有趣的机制. 比如遇到神仙, 百姓, 强盗, 行侠仗义, 刺客.
5. 地图的六边形, 要改进一下, 之前的生成里面uv计算不够平滑.


<div class="date">2023年5月19日</div>

1. 代码清理了一下, 写一段主线, 手生了啊.
2. 今天看到一个来自godotengine.org的访问, 让我好生诧异, 要被诏安了?

<div class="date">2023年5月18日</div>

1. 主线, 发挥一下家国情怀, 做不了诸葛亮, 做做罗贯中, 也未尝不好.
2. 技能的话, 目前用了三十六计和一些成语, 莫非要成语大师?
3. 城市, 进入战斗状态, 则内政停止结算, 且会到破坏
4. 后面真的代码多了的话, 要适配一下gdextension机制. 这个算后话.


在油管上看到一个几个月前的老贴, 主题是为啥不使用godot的原因. 我没细看视频, 毕竟他说的不对. 但我决定看看评论, 看看雪亮的群众之眼, 会作何点评. 果然, 大部分都是反驳楼主的, 并有理有据的说了一堆理由. 那我就放心了. 

我对其中有一条并不认同. 他说, godot的优势是他的语言, 很像python, 他非常喜欢python, 且python是他第一个学会的语言(first language). 这不巧了吗. 这时候, 我就很激动了, 像python, 哪有就是python来得正义!!! 来吧, 这里才是你的温暖港湾. 

官方解释了他为啥要自己做一个语言, 首先, 肯定确定, 是因为开发者喜欢写语言. 其次, 他们尝试了各种语言, 要么语言机制不好, 要么就是性能太差. 语言机制不足, 想必指lua; 而性能太差, 毫无疑问是python. 不愧是性能榜上拖后腿的扛把子.

但那都是很久以前的事了, python现在性能已经好很多了. 虽然还是扛把子, 但真的好很多了. 语言机制,真的很优秀. 且受众广泛, 且很多人都会. 来尝试一下呗. 毫无门槛, 熟悉的语言, 同样的配方.

其实吧, 虽然学一个语言很加单, 就是一些接口调用. 但是用了这个语言,你会失去很多熟悉的东西,现成的东西,最可惜的,是失去python整个社区. 所以, 我觉得能用原生的python是最佳的方案. python作为胶水语言,是十分称职的.

对于这个胶水层的一点点看法. 选择一个语言, 就等于选择了这个语言的框架, 很多框架设计往往很重. 做复杂的框架, 对作者自己也是个折磨, 改一点点也是伤筋动骨. 我只想提供一个引擎的访问接口, 减少焦虑, 减少额外信息.

补充一条: python 3.12对线程的gil有了一点改进, 每一个线程有单独的锁了. 于是于, 多线程那条理由, 也有所变化.

<div class="date">2023年5月17日</div>

把机器人玩家的创建, 初始化, 旗帜颜色搞好了. 接下来细化机器人的发展. 后面初始数值应该在做一些差异化.
玩家的行为, 发展和扩张,外交,纵横, 发展比较简单, 就是内政搞搞, 应该不用太多的变化. 但是扩张, 外交等行为, 理应搞得骚一点.
所以, 主节点应该是打仗!

然后军队移动躲避过程里, 还是有原地转圈的现象. 看代码是有一些小缝隙里面, 会导致原地转圈. 难以避免这些缝隙的存在.
...
1. 机器人玩家的AI, 需要和剧本相结合. 
2. 还是主线, 主线好才是真的好啊. 改编一个成功故事, 三国只要不是刘备就可以的吧?
...
2. 建设
3. AI
4. 主线剧本要搞一个高级一点的了, 没必要放三国吧
5. 砸技能的时候, 真的是砸了一个技能名字

<div class="date">2023年5月12日</div>

1. 指令面板的子界面, 保持单一, 点第二次会关闭前一个
...
1. 把框选跟原来的点选功能暂时分开, 感觉点选的功能不必保留
2. 把单位身上的菜单交互去掉, 菜单这个方式还是太丑了
3. 空地的菜单也去掉, 把建设的入口放到顶栏去.
4. 群体指令做了, 这样攻城的操作是方便了, 但是就显得行军不够细腻. 队伍内的单位的行军细节, 快要提上来了.
5. 继续做AI, 这样就可以有攻防了.
